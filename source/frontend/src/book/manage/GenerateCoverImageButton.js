...
const GenerateCoverImageButton = ({onClickGenerateButton, defaultQuery, ...props}) => {
  ...
  return (
    <>
    <Box onClick={()=>{setIsDialogOpend(true);setQuery(defaultQuery)}} sx={{float: "left", backgroundColor: "cornflowerblue", width: "63px", height: "25px", padding: "8px", borderRadius: "5px", cursor: "pointer", "&:hover": {opacity: 0.80}}}>
    ...
    <Dialog open={isDialogOpend} onClose={()=>{setIsDialogOpend(false);}}>
      <DialogTitle sx={{color: "black", fontWeight: "bolder", fontFamily: "BMDfont"}}>책 표지 이미지 생성</DialogTitle>
      <DialogContent>
        <Stack>
            <StyledTextField
                name="query"
