...
public class AddBookShelfBookEndPoints {
    ...
    public ResponseEntity<AddBookShelfBookResDto> addBookShelfBook(@RequestBody AddBookShelfBookReqDto reqDto) {
        try {
            ...
            bookShelfBook.setBookShelfId(reqDto.getBookShelfId());
            bookShelfBook.setBookId(reqDto.getBookId());
            
            // [2] BookShelfBookAdded 이벤트를 생성된 BookShelfBook로 발생시킴
            BookShelfBookAdded event = new BookShelfBookAdded(bookShelfBook);
            applicationEventPublisher.publishEvent(event);
            // [3] 생성된 BookShelfBook 객체의 ID를 반환
            AddBookShelfBookResDto responseDto = new AddBookShelfBookResDto(bookShelfBook);
