...
public class GenerateContentEndPoints {
    ...
    public ResponseEntity<GenerateContentResDto> generateContent(@RequestBody GenerateContentReqDto reqDto) {
        try {
            ...
            content.setIndexId(reqDto.getIndexId());

            // [2] ContentImageGenerationRequested 이벤트를 생성된 Content 기반으로 발생시킴
            ContentImageGenerationRequested imageEvent = new ContentImageGenerationRequested(content);
            applicationEventPublisher.publishEvent(imageEvent);

            // [3] ContentGenerationRequested 이벤트를 생성된 Content 기반으로 발생시킴
            ContentGenerationRequested generationEvent = new ContentGenerationRequested(content);
            applicationEventPublisher.publishEvent(generationEvent);
